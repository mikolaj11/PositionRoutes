...
\caption{\protect\subref{fig:RSKa} The original tableau $P$.
	...
	corresponds to an insertion of the number $18$. The numbers next to the arrows describe the bumped boxes with the numbers.\protect\subref{fig:RSKc} The
...
The RSK algorithm is an important tool in algebraic combinatorics, especially in the context of Littlewood--Richardson coefficients and plactic monoid\cite[Lecture 4]{Fulton1991}. 
...
The RSK algorithm applied to the sequence of independent and identically distributed random variables with the uniform distribution $U(0,1)$ on the unit interval $(0,1)$ generates the \emph{Plancherel measure} on Young diagrams \cite[CHapter 1.8]{Romik2015}. The Plancherel measure is an important element of the representation theory of the symmetric group because it describes how the left regular representation decomposes into irreducible componennts \cite[Chapter 3.3]{Fulton1991}.

The Ulam--Hammersley problem appears in \cite[CHapter 1.1]{Romik2015} and concerns the typical length of the longest increasing subsequence in a random permutation. This problem corresponds to the problem of typical length of the first row in the Young tableau obtained by the RSK algorithm from the sequence of independent random variables $\{X_j\}_{j=1}^n$ with the uniform distribution $U(0,1)$ on the unit interval $(0,1)$.
...
Romik and Śniady \cite{Romik2016} considered the limit shape of the bumping routes obtained when applying an RSK insertion step with a fixed number $w$ to an existing insertion tableau $P(X_1, X_2, \ldots, X_n)$ obtained from a random finite sequence. In \cite{Romik2015a} they considered also the limit shape of \emph{jeu de taquin} obtained from the recording tableau $Q(w, X_1, X_2, \ldots, X_n)$ made from a random finite sequence preceded by a fixed number $w$. 
...
\end{align*}
The function $F_{SC}$ is the cumulative distribution function of the \emph{Wigner's semicircle distribution}. The function $\Omega_{\star}$ is the limit shape of the Young Tableau obtained from the \emph{Plancherel growth process}. This curve is called the Logan--Shepp--Vershik--Kerov curve \cite{LoganShepp1977}\cite{VershikKerov1986}. The function $x \mapsto \big(u(x), v(x)\big)$ is a special parameterisation of the function $\Omega_{\star}$ and describes the limit position of the new box in the RSK insertion step applied with the number $x$ to the random Young Tableau \cite[Chapter 5.1]{Romik2015a}. The function $G\colon [0,1] \to \mathbb{R}_+^2$ is the function $\big(u(x), v(x)\big)$ rotated by 45 degrees (see \cite[Chapter 5.1]{Romik2015a}). Moreover the function $G(x)$ is continuous and increasing in the first coordinate and decreasing in the second coordinate.
\begin{figure}
 \begin{center}
\verker
\end{center}
...
\begin{theorem}
\label{thm:romthm}
For each $x\in[0,1]$ the position $\square_n(x)$, after scaling by $\frac{1}{\sqrt{n}}$, converges in probability to a specific point $G(x)\in [0,2]^2$, when $n$ tends to infinity: $$\frac{\square_n(x)}{\sqrt{n}}\overset{p}{\to}G(x).$$ 
\end{theorem}
...
For example the function $G$ is \emph{increasing with respect to the relation $\prec$}, i.e. if $x_1\leq x_2$ then $G(x_1)\prec G(x_2)$. Likewise, from a property of the RSK insertion step, for each natural number $n\in \mathbb{N}$ the function $\square_n$ is increasing with respect to the relation $\prec$, i.e. if $x_1\leq x_2$ then $\square_n(x_1)\prec \square_n(x_2)$.

\subsection{The random increasing sequence}
\label{incseq}
Let $w\in(0,1]$ be a fixed number and let $\{X'_j\}_{j=1}^{m'}$ be a finite sequence of independent random variables with the uniform distribution $U(0,w)$. For $j\in\{1, 2, \ldots, m'\}$ we define $z(j)$ as $j$-th order statistic, i.e. the $j$-th smallest number among $X'_1, X'_2, \ldots, X'_{m'}$. The sequence $\{z(j)\}_{j=1}^{m'}$ contains all elements of the sequence $\{X'_j\}_{j=1}^{m'}$ in the ascending order. The sequence $\{z(j)\}_{j=1}^{m'}$ will be called a \emph{random increasing sequence with the uniform distribution on the interval $[0,w]$}. \\

In addition, the sequence $\{X'_j\}_{j=1}^{m'}$ is a some permutation $\Pi=\left(\Pi_1, \Pi_2, \ldots, \Pi_{m'}\right)$ of the sequence $\{z(j)\}_{j=1}^{m'}$. 
$$\Big\{X'_j\Big\}_{j=1}^{m'}=\Big\{z(\Pi_j)\Big\}_{j=1}^{m'}=z\circ\Pi,$$
where $z$ is the function that acts pointwise on every element of the permutation $\Pi$. The permutation $\Pi$ is the random permutation with the uniform distribution. We see that a finite sequence of independent random variables with the uniform distribution $U(0,w)$ has the same distribution as a finite random increasing sequence with the uniform distribution on the interval $[0,w]$ permuted by a random permutation with the uniform distribution.
...
First, we prove only the convergence on the interval $[1, \infty)$, i.e. we prove that for each number $T\in[1,\infty)$ and for each $\epsilon>0$
...
\begin{align*}
\Pos_n\Big(\floor{Tn}\Big)&=\boxi_w\Big(P\left(X_1, \ldots, X_n, w, X_{n+1}, \ldots, X_{m}\right)\Big)\\&=\boxi_w\Big(P\left(X_1, \ldots, X'_{n'}, w, X'_{n'+1}, \ldots, X'_{m'}\right)\Big).
\end{align*}
...
\begin{align*}
\lim_{n\to\infty}\frac{m'}{n'}&= \lim_{n\to\infty}1+\frac{m'-n'}{n'}\\&=1+\lim_{n\to\infty}\frac{m'-n'}{m-n}\frac{1}{\frac{n'}{n}}\frac{m-n}{n}\\&=1+\lim_{n\to\infty}\frac{m'-n'}{m-n}\frac{1}{\frac{n'}{n}}\left(\frac{m}{n}-1\right)\\&=1+\frac{w}{w}\left(T-1\right)\\&=T
\end{align*}
and
\begin{align*}
...
Similarly, the function $z$ acts on the insertion tableau by acting on each box individually. Since the function $z$ is increasing, the function $z$ by acting on the input of the RSK algorithm change pointwise the numbers in the insertion tableau. Then
\begin{align*}
...
\end{align*}
Now, by changing the number $n'+1$ to the number $n'+\frac{1}{2}$ we get the inverse permutation $\Pi^{-1}$ 
\begin{align*}
...
\end{align*}
Since the function $z$ is increasing, the function $z$ by acting on the input of the RSK algorithm not change the content of the recording tableau. Using the function $z$, we will get a sequence of independent random variables with the uniform distribution $U(0,1)$ 
\pagebreak[2]
\begin{align*}
...
\end{align*}
Using little-$o$ notation this means that for each $x\in[0,1]$ and for each $\epsilon>0$ 
...
\end{align}
Let $\epsilon>0$.\\
The function $G(x)$ is a continuous function in the point $A=\frac{1}{T}$. This means that there exists $\delta>0$ that if $|x-A| < 2\delta$ then $\left\|G(x)-G(A)\right\|<\epsilon$. Thus 
\begin{align}
(-\epsilon,\epsilon)\prec G(x)-G(A)\prec (\epsilon,-\epsilon).
\label{conti}
\end{align} 
In addition, almost surely $A_n \to A$. Then also $A_n \overset{p}{\to} A$. Using little-$o$ notation we have:
\begin{align}
\mathbb{P}(A-\delta<A_n< A+\delta)=1-o(1). 
\label{convt}
\end{align}

If $A_n<A+\delta$ and $\left\| G_n(A+\delta)-G(A+\delta) \right\| \leq \epsilon$ then:
\begin{align*}
G_n(A_n)-G(A)&\prec G_n(A+\delta)-G(A) \tag{by \ref{ineq}}\\&=G_n(A+\delta)-G(A+\delta)+G(A+\delta)-G(A)\\&\prec (\epsilon,-\epsilon)+(\epsilon,-\epsilon) \tag{by \ref{convx},\ref{conti}}\\&=(2\epsilon,-2\epsilon).
\end{align*}
Analogously, if $A-\delta<A_n$ and $\left\| G_n(A-\delta)-G(A-\delta) \right\|\leq \epsilon$ then:
\begin{align*}
G_n(A_n)-G(A)&\succ (-2\epsilon,2\epsilon).
\end{align*}
Thus if $\left\| G_n(A+\delta)-G(A+\delta) \right\| \leq \epsilon$ and $\left\| G_n(A-\delta)-G(A-\delta) \right\|\leq \epsilon$ and \linebreak[4]{$\left| A_n-A\right|<\delta$} then:
\begin{align*}
\left\|G_n(A_n)-G(A)\right\|< \left\| (2\epsilon,-2\epsilon) \right\| <4 \epsilon .
\end{align*}
Thereby:
\begin{align*}
&\mathbb{P}\bigg(\left\|G_{n}(A_n)-G(A)\right\| > 4\epsilon\bigg)\leq \\&\leq  \mathbb{P}\Big(\left\|G_n(A+\delta)-G(A+\delta)\right\|> \epsilon \text{\hspace{15pt}or\hspace{12pt}}\left\|G_n(A-\delta)-G(A-\delta)\right\|> \epsilon \text{\hspace{15pt}or\hspace{12pt}}\left| A_n-A\right|\geq \delta\Big)\\&\leq  \mathbb{P}\Big(\left\|G_n(A+\delta)-G(A+\delta)\right\|> \epsilon \Big) +\mathbb{P}\Big( \left\|G_n(A-\delta)-G(A-\delta)\right\|> \epsilon\Big)+ \mathbb{P}\Big(\left| A_n-A\right|\geq \delta\Big)\\&=o(1).\tag{by \ref{smallo},\ref{convt}}
\end{align*}
Then for each $T\in[1,\infty)$ and each $\epsilon>0$ we have:
$$\lim_{n\to\infty}\mathbb{P}\bigg(\left\|G_{n}(A_n)-G(A)\right\| > 4\frac{\epsilon}{4\sqrt{T}}\bigg)=0,$$
$$\lim_{n\to\infty}\mathbb{P}\left(\left\|\frac{\Pos_n\left(\floor{Tn}\right)}{\sqrt{wn}}-H\left(T\right)\right\|>\epsilon\right)=0.$$
...
In this subsection we prove the uniform convergence on the interval $[1, \infty)$. 
\begin{proof}
We know that $G_n(A_n)$ converges in probability to the limit $G(A)$, when $n$ tends to infinity for $A=\frac{1}{T}\in[\frac{1}{R},1]$.\\
The function $G$ is increasing with respect to the relation $\prec$. In addition, $G$ is a continuous function on the closed interval $[\frac{1}{R},1]$. Then from the Heine--Cantor theorem the function $G(x)$ is uniformly continuous on the interval $[\frac{1}{R},1]$. 
This means that for each $\epsilon>0$ there exists $\delta>0$ that for each $x, A\in[\frac{1}{R}, 1]$ if $|x-A| < \delta$ then $\left\|G(x)-G(T)\right\|<\epsilon$. Thus 

\begin{align*}
(-\epsilon,\epsilon)\prec G(x)-G(A)\prec (\epsilon,-\epsilon).
\end{align*}

Let $\delta>0$. We split the interval $[\frac{1}{R}, 1]$ into $p$ equal intervals of length $\frac{1-\frac{1}{R}}{p}<\delta$ for a some natural number $p$. Let $x_0, x_1, \ldots, x_p$ denote the endpoints of these intervals. Let $A_n\in[\frac{1}{R},1]$. We assume there exists the limit $\lim_{n\to\infty}A_n=A$. We choose a number $n_0$ so that for a some number $k\in\{0, 1, \ldots, p\}$
\begin{align*}
A_n, A\in(x_k, x_{k+1})\text{ for each number }n>n_0 \text{ or }A=x_k 
\end{align*}
and
\begin{align*}
||G_n(x_k) -G(x_k)||<\epsilon \text{ for all }n>n_0\text{ and all } k\in\{0, 1, \ldots, p\}. 
\end{align*}
Thereby if $A_n, A\in(x_k, x_{k+1})$ then
\begin{align*}
G_n(A_n)-G(A)&\prec G_n(x_{k+1})-G(A) \tag{by \ref{ineq}}\\&\prec(\epsilon,-\epsilon)+G(x_{k+1})-G(A)\\&\prec (\epsilon,-\epsilon)+(\epsilon,-\epsilon) \tag{by \ref{ineq}}\\&=(2\epsilon,-2\epsilon).
\end{align*}
and analogously
\begin{align*}
G_n(A_n)-G(A)&\succ (-2\epsilon,2\epsilon).
...
\left\|G_n(A_n)-G(A)\right\|< \left\| (2\epsilon,-2\epsilon) \right\| <4 \epsilon .
\end{align*}
Obviously if $A=x_k$ for a some number $k\in\{0, 1, \ldots, p\}$ then the inequality also is fulfil. Then for each $A\in[1,R]$ and each $\epsilon>0$ we have:
$$\lim_{n\to\infty}\mathbb{P}\bigg(\sup_{A\in[\frac{1}{R},1]}\sqrt{T}\left\|G_{n}(A_n)-G(A)\right\| > 4\sqrt{T}\epsilon\bigg)=0.$$
Using the inequality $T\leq R$ we obtain

$$\lim_{n\to\infty}\mathbb{P}\bigg(\sup_{A\in[\frac{1}{R},1]}\sqrt{T}\left\|G_{n}(A_n)-G(A)\right\| > 4\sqrt{R}\epsilon\bigg)=0,$$
$$	\lim_{n\to\infty}\mathbb{P}\Bigg(\sup_{T\in[1,R]} \left\|\frac{\Pos_n\left(\floor{Tn}\right)}{\sqrt{wn}}-H\left(T\right)\right\|>4\sqrt{R}\epsilon\Bigg)=0.$$
Thus for each $\epsilon>0$ occur
$$	\lim_{n\to\infty}\mathbb{P}\Bigg(\sup_{T\in[1,R]} \left\|\frac{\Pos_n\left(\floor{Tn}\right)}{\sqrt{wn}}-H\left(T\right)\right\|>\epsilon\Bigg)=0.$$
\end{proof}

\section{Asymptotic behavior of the function $H$}

In this chapter we will describe the asymptotic behavior of the function $H\big(T)$ when $T$ tends to infinity. Let $A=\frac{1}{T}$. For this purpose, we will use the formulas given in \cref{wzory}.

$$H(T)=H\big(1/A\big)=\sqrt{\frac{1}{A}}G(A)=\frac{1}{2\sqrt{A}}\bigg(v(A)+u(A), v(A)-u(A)\bigg).$$
We define the function $f:[0,2]\to [0,\sqrt{\frac{3\pi}{2}}]$ by
$$f(x)=\sqrt[3]{\frac{3\pi}{2} F_{sc}(x^2-2)}.$$
The function $F_{sc}$ is invertible, so the function $f$ also is invertible and the above formula can be transformed into the form
$$f^{-1}(y)=\sqrt{F_{sc}^{-1}\bigg(\frac{2y^3}{3\pi}\bigg)+2}.$$
...
We use the formulas for the coefficients of the series of the inverse function given by Morse and Feshbach in \cite{Morse1953} and we determine the series of inverse function $f^{-1}(y)$.
...
&=O\left(x^{\frac{9}{3}}\right)+\frac{4}{3\pi}\Bigg(\bigg(\frac{3\pi}{2}x\bigg)^{\frac{1}{3}}+\frac{1}{40}\bigg(\frac{3\pi}{2}x\bigg)^{\frac{3}{3}}+\frac{81}{22400}\bigg(\frac{3\pi}{2}x\bigg)^{\frac{5}{3}}+O\left(x^{\frac{7}{3}}\right)\Bigg)^3+\cdots\\
&=O\left(x^{\frac{9}{3}}\right)+\frac{4}{3\pi}\Bigg(\bigg(\frac{3\pi}{2}x\bigg)^{\frac{1}{3}}+\frac{1}{40}\bigg(\frac{3\pi}{2}x\bigg)^{\frac{3}{3}}+\frac{81}{22400}\bigg(\frac{3\pi}{2}x\bigg)^{\frac{5}{3}}\Bigg)^3+\cdots\\
...
Thus
\begin{align*}
\frac{v(A)+u(A)}{2\sqrt{A}}&=\sqrt{A}+\frac{1}{10}\left(\frac{3\pi}{2}\right)^{\frac{2}{3}}A^{\frac{7}{6}}+\frac{1}{56}\left(\frac{3\pi}{2}\right)^{\frac{4}{3}}A^{\frac{11}{6}}+O\left(A^{\frac{15}{6}}\right)
\end{align*}
and
...
\begin{align*}
&\frac{v(A)-u(A)}{2\sqrt{A}}\\&=
\frac{2}{\sqrt{A}}-\left(\frac{3\pi}{2}\right)^{\frac{2}{3}}\hspace{-5pt}A^{\frac{1}{6}}+A^{\frac{3}{6}}-\left(\frac{3\pi}{2}\right)^{\frac{4}{3}}\hspace{-3pt}\frac{A^{\frac{5}{6}}}{20}+\left(\frac{3\pi}{2}\right)^{\frac{2}{3}}\hspace{-3pt}\frac{A^{\frac{7}{6}}}{10}-\left(\frac{3\pi}{2}\right)^{\frac{6}{3}}\frac{11A^{\frac{9}{6}}}{1400}+\left(\frac{3\pi}{2}\right)^{\frac{4}{3}}\hspace{-3pt}\frac{A^{\frac{11}{6}}}{56}+O\left(A^{\frac{13}{6}}\right)
\end{align*}
We see that the function $H$ behaves asymptotically like the function $(\frac{1}{\sqrt{T}}, 2\sqrt{T})$. The function $H$ describes the asymptotic position of the box with a fixed number $w$. The $x$-coordinate of this position after scaling tends to the $OY$ axis. If we did not scale it, would it also be similar? Will the box with the number $w$ be move to the first column? We expect that almost surely yes. How long will we have to wait for this? We expect that if the number $w$ appears in the RSK algorithm after $n$ steps, the waiting time for the number in the first column is $O(n^2)$. Is this really true? More formally, let $\{X_j\}_{j=1}^\infty$ be a sequence of independent random variables with the uniform distribution $U(0,1)$. What is the probability that the box with a fixed number $w$ will be in the first column in the insertion tableau $P(x_1, \ldots, X_n, w, X_{n+1}, \ldots, X_{\floor{Tn^2}})$ depending on parameter $T$?
