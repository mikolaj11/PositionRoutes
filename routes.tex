...
\Omega_{\star}(x)&=\frac{2}{\pi}\bigg(\sqrt{1-x^2}+x\sin^{-1}\Big(\frac{x}{2}\Big)\bigg)&\big(-2\leq x \leq 2\big),\\
u(x)&=F_{SC}^{-1}(x),\\
v(x)&=\Omega_{\star}(u(x)),\\
...
The function $G(x) \colon [0,1]\to[0,2]^2$ is a certain special parameterization of the Logan--Sheep--Vershik--Kerov curve rotated by 45 degrees (see \cite[Chapter 5.1]{Romik2015a}). \cref{fig:gfunction} shows the graph of the function $G(x)$. Moreover the function $G(x)$ is continuous and increasing in the first coordinate and decreasing in the second coordinate. \\
