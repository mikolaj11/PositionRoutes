...
Full version of this paper will be published in the other journal.
...
The RSK algorithm is an important tool in algebraic combinatorics, especially in the context of Littlewood--Richardson coefficients and plactic monoid\cite{Fulton1991}. The RSK algorithm and operations on the words are crucial to understanding the Littlewood--Richardson coefficients.\\\\ 
...
\subsection{Result of Romik and Śniady}
In the proof of Theorem \ref{thm:mainthm} we will need the following theorem proved by Romik and Śniady\big(Theorem 5.1 in \cite{Romik2015a}\big). 
\begin{theorem}
\label{thm:romthm}
Let $\{X_j\}_{j=1}^\infty$ be a  sequence of independent random variables with the uniform distribution $U(0,1)$. Let $\square_n(x)\in \mathbb{N}^2$ denote the position of the new box in the insertion tableau $P(X_1, \ldots, X_n, x)$ when we apply the RSK insertion step for the number $x\in[0,1]$ to the previously obtained tableau $P(X_1, \ldots, X_n)$. Then this position, after scaling, converges in probability to the limit curve $G(x)$: $$\frac{\square_n(x)}{\sqrt{n}}\overset{p}{\to}G(x),$$ where $G(x):[0,1]\to[0,2]^2$ is the Logan--Sheep--Vershik--Kerov curve rotated by 45 degrees.
\begin{figure}
 \begin{center}
\verker
\end{center}
...
\label{fig:gfunction}
\end{figure}
Figure~\ref{fig:gfunction} shows the graph of the function $G(x)$. Moreover the Logan--Sheep--Vershik--Kerov curve \cite{LoganShepp1977}\cite{VershikKerov1986} is a Lipschitz function with constant $1$. After rotation, this means that for each $x\geq 0$ and $\epsilon>0$ where $x+\epsilon\leq 1$ we have:
$$G(x)-\epsilon(1,-1)\prec G(x+\epsilon) \prec G(x)+\epsilon(1,-1),$$
where the relation $\prec$ is an order on the plane defined as follows:
$(x_1,y_1)\prec(x_2,y_2)$ only and only if $x_1<x_2$ and $y_1>y_2$. The relation $\prec$ retains the order of the points on the plane along the line $y=-x$. In addition, the function $G(x)$ is also monotonic with respect to the relation $\prec$ on the plane.
\end{theorem}
\section{The main result}
The main result is Theorem \ref{thm:mainthm} describing the asymptotic behavior of the box with a fixed number. When the number of boxes tends to infinity then the trajectory of the box converges in probability to the function $$H(A)=\sqrt{A}G(\frac{1}{A}).$$ The curve $H(A)$ is also the limit shape of bumping routes\cite{Romik2016} in the RSK algorithm. Figure \ref{fig:trzy} shows the graph of the function $H(x)$ and the experimentally determined position of the box with the number $w=0.5$ for $n=400$.
\begin{figure}
\trzy
...
$$\Pos_n\left(j\right)=\boxi_w\biggl(P\left(X_1, \ldots, X_n, w, X_{n+1}, \ldots, X_j\right)\biggr)$$ for $j\in\{n+1, n+2, \ldots\}$, where for a tableau $P$ the value $\boxi_w(P)$ is the coordinates in $\mathbb{N}^2$ of the box with the number $w$. 

\begin{theorem}
\label{thm:mainthm}
For each number $A\in(1,\infty)$ the random variable $\Pos_n(\floor{An})$, after scaling by $\sqrt{wn}$, converges in probability to the limit $H(A)$, when $n$ tends to infinity: $$\forall_{\epsilon>0} \lim_{n\to\infty}\mathbb{P}\Bigg( \left\|\frac{\Pos_n\left(\floor{An}\right)}{\sqrt{wn}}-H\left(A\right)\right\|>\epsilon\Bigg)=0.$$
\end{theorem}
\begin{proof}
...
Now we will use this observation in the proof. Let $m=\floor{An}$. The probability that the same number will occur twice in the sequence $(w, X_1, X_2, \ldots)$ is equal to 0, hence without losing generality we assume that the numbers $w, X_1, X_2, \ldots$ are all different. Let the sequence $\{X'_j\}_{j=1}^\infty$ be the subsequence of the sequence $\{X_j\}_{j=1}^\infty$ containing all elements of the sequence $\{X_j\}_{j=1}^\infty$, which are less than $w$. The sequence $\{X'_j\}_{j=1}^{\infty}$ is a sequence of independent random variables with the uniform distribution $U(0,w)$. Let $n'=n'(n)$ and $m'=m'(m)$ denote the number of elements respectively of the sequences $\{X_j\}_{j=1}^n$, $\{X_j\}_{j=1}^m$ which are smaller than $w$. Then there is an equality:
\begin{align*}
...
From the Strong Law of Large Numbers theorem \cite{Durrett2019}, we know that if $n$ tends to infinity, then the following limits exist with probability $1$:
$$
\begin{matrix}
...
\begin{align*}
\frac{\square_{m'}(x)}{\sqrt{m'}}\overset{p}{\to} G(x)
\end{align*}
...
\end{align*}
We know that with probability $1$, if $n$ tends to infinity then
$$T_n \to T.$$ 
Then for each $x\in(0,1)$ with probability $1$ for each $\epsilon>0$ exists $n_0$ that for each $n>n_0$ occur:
$$T-\epsilon< T_n < T + \epsilon.$$
In addition, for each $x$ occur:
$$G_{m'}(x)\overset{p}{\to} G(x),$$
$$\mathbb{P}\bigg(G(x)-\epsilon(1,-1)\prec G_{m'}(x) \prec G(x)+\epsilon(1,-1)\bigg)=1-o(1).$$
From Theorem \ref{thm:romthm} we have:
$$\mathbb{P}\bigg(G_{m'}(T_n)\prec G_{m'}(T + \epsilon)\prec G_{m'}(T) + \epsilon(1,-1)\prec G(T) + 2\epsilon(1,-1)\bigg)=1-o(1)$$
and
$$\mathbb{P}\bigg(G_{m'}(T_n)\succ G_{m'}(T - \epsilon)\succ G_{m'}(T) - \epsilon(1,-1)\succ G(T) - 2\epsilon(1,-1)\bigg)=1-o(1).$$
Thereby:
$$\mathbb{P}\bigg(\left\|G_{m'}(T_n)-G(T)\right\|<2\sqrt{2}\epsilon<4\epsilon\bigg)=1-o(1).$$
Then for each $A\in(1,\infty)$ for each $\epsilon>0$ we have:
 $$ \mathbb{P}\bigg(\left\|\frac{\Pos_n\left(\floor{An}\right)}{\sqrt{Awn}}-G\left(T\right)\right\|<4\frac{\epsilon}{4\sqrt{A}}\bigg)=1-o(1),$$
$$\mathbb{P}\bigg(\left\|\frac{\Pos_n\left(\floor{An}\right)}{\sqrt{wn}}-\sqrt{A}G\left(\frac{1}{A}\right)\right\|<\epsilon\bigg)=1-o(1).$$
