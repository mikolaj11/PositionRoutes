...
\begin{abstract}
We investigate evolution in the time of the fixed box in the insertion tableau, when the Robinson-Schensted-Knuth algorithm is applied to a finite sequence of random variables. When the length of the sequence tends to infinity, a typical trajectory after scaling converges in probability to the some limit.
\end{abstract}
...
Benjamin Logan together and Lawrence Shepp in \cite{} and independently Anatoly Vershik together and Sergei Kerov in \cite{} solved to the Ulam-Hammersley problem concerning typical lenght of a longest increasing subsequence in random permutations. The longest increasing subsequence also describes the length of firs row in the insertion tableu obtained when we apply the RSK algorithm to the random finite sequence\\\\
...
In \cite{RS15} they considered limit shape of jeu de taquin obtained from insertion tableau made from the random finite sequence preceded by a fixed number. \\\\
We can merge the cases considered by Dan Romik and Piotr Śniady and consider insertion tableau obtained by using an RSK algorithm applied to a random finite sequence containing a fixed number at some index. The box with fixed number can be sliding by the bumping routes.\\\\
