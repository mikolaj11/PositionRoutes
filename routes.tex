...
Let $\epsilon>0$.\\
In addition, the function $G(x)$ is a continuous function in the point $T\in(0,1)$. This means that exists $\delta>0$ that if $|x-T| < 2\delta$ then $\left\|G(x)-G(T)\right\|<\epsilon$. Thus $$\epsilon(1,-1)\prec G(x)-G(T)\prec \epsilon(1,-1).$$ Moreover with probability $1$:
$$T_n \to T.$$ 
Then there exists $n_0$ such that for each $n>n_0$
$$\mathbb{P}(T-\delta<T_n< T+\delta)=1.$$
If $T_n<T+\delta$ and $\left\| G_n(T+\gamma)-G(T+\gamma) \right\| < \epsilon$ then:
\begin{align*}
G_n(T_n)-G(T)&\prec G_n(T+\gamma)-G(T)\\&=G_n(T+\gamma)-G(T+\gamma)+G(T+\gamma)-G(T)\\&\prec\epsilon(1,-1)+\epsilon(1,-1)\\&=2\epsilon(1,-1)
\end{align*}
Analogously if $T-\delta<T_n$ and $\left\| G_n(T+\gamma)-G(T+\gamma) \right\|<\epsilon$ then:
\begin{align*}
G_n(T_n)-G(T)&\succ G_n(T-\gamma)-G(T)\\&=G_n(T-\gamma)-G(T-\gamma)+G(T-\gamma)-G(T)\\&\succ -\epsilon(1,-1)-\epsilon(1,-1)\\&=-2\epsilon(1,-1)
\end{align*}
Thus
$$\left\|G_n(T_n)-G(T)\right\|<2 \epsilon \left\| (1,-1) \right\| <4 \epsilon$$
Thereby for sufficiently large values of $n$ we have:
\begin{align*}
1&\geq\mathbb{P}\bigg(\left\|G_{n}(T_n)-G(T)\right\|\leq 4\epsilon\bigg)\\&\geq \mathbb{P}\Big(\left\|G_n(T+\gamma)-G(T+\gamma)\right\|\leq \epsilon\text{\hspace{10pt}and\hspace{10pt}} \left\|G_n(T-\gamma)-G(T-\gamma)\right\|\leq \epsilon\Big)\\&\geq \mathbb{P}\Big(\left\|G_n(T+\gamma)-G(T+\gamma)\right\|\leq \epsilon \Big) +\mathbb{P}\Big( \left\|G_n(T-\gamma)-G(T-\gamma)\right\|\leq \epsilon\Big)-1\\&=1-o(1)+1-o(1)-1  \\&=1-o(1),
\end{align*}
