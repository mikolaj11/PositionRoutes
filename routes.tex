...
\title[Dynamic limit of RSK]{Dynamic limit \\ of Robinson--Schensted--Knuth algorithm}
...
\address{\addressmark{1}Interdisciplinary PhD School “Academia Copernicana”, Faculty of Mathematics and Computer Science, Nicolaus Copernicus University in Toruń, ul.~Chopina 12/18, 87-100 Toruń, Poland}
...
\caption{\protect\subref{fig:RSKa} The original tableau $P$.
	...
	corresponds to an insertion of the number $18$. The numbers next to the arrows describe the rearrenged boxes with the numbers.\protect\subref{fig:RSKc} The
...
In this article we will investigate the position of the box with a selected number and how its position is changing over time. 



\subsection{Motivations}
The RSK algorithm is an important tool in algebraic combinatorics, especially in the context of Littlewood--Richardson coefficients and plactic monoid\cite{Fulton1991}. 

\medskip

For many years mathematicians have been studying the asymptotic behavior of the insertion tableau when we apply the Robinson--Schensted--Knuth algorithm to a random input. In the following paragraphs we will see several examples of such considerations. The RSK algorithm applied to the sequence of independent and identically distributed random variables with the uniform distribution on the interval $(0,1)$ generates the \emph{Plancherel measure} on Young diagrams \cite{Romik2015}. The Plancherel measure is an important element of the representation theory because it describes how the left regular representation decomposes into irreducible componennts \cite[Chapter 3.3]{Fulton1991}.

The Ulam--Hammersley problem appears in \cite{Baik1999} and concerning the typical length of the longest increasing subsequence in a random permutation corresponds to the problem concerning the typical length of the first row in the Young tableau obtained by the RSK algorithm from the sequence of independent random variables $\{X_j\}_{j=1}^n$ with the uniform distribution $U(0,1)$ on the interval $(0,1)$.
...
\label{wzory}
We define the functions $F_{SC}, \Omega_{\star}, u, v, G$ by:
\begin{align*}
F_{SC}(x)&=\frac{1}{2}+\frac{1}{\pi}\bigg(\frac{x\sqrt{4-x^2}}{4}+\sin^{-1}\Big(\frac{x}{2}\Big)\bigg)\hspace{30pt}&\big(-2\leq x \leq 2\big),\\
\Omega_{\star}(x)&=\frac{2}{\pi}\bigg(\sqrt{4-x^2}+x\sin^{-1}\Big(\frac{x}{2}\Big)\bigg)&\big(-2\leq x \leq 2\big),\\
u(x)&=F_{SC}^{-1}(x),\\
v(x)&=\Omega_{\star}(u(x)),\\
G(x)&=\Big(\frac{v(x)+u(x)}{2},\frac{v(x)-u(x)}{2}\Big).
\end{align*}
\begin{figure}[h]
 \begin{center}
\verker
...
\label{fig:gfunction}
\end{figure}
\cref{fig:gfunction} shows the graph of the function $G(x)$. \\
The function $F_{SC}$ is the cumulative distribution function of the \emph{semicircle distribution}. The function $\Omega_{\star}$ is the limit shape of the Young Tableau obtained from the \emph{Plancherel growth process}. This curve is called the Logan--Sheep--Vershik--Kerov curve. The function $\big(u(x), v(x)\big)$ is a special parameterisation of the function $\Omega_{\star}(x)$ and describes the limit position of the new box in the RSK insertion step applied with the number $x$ to the random Young Tableau. The function $G(x) \colon [0,1]\to[0,2]^2$ is the function $\big(u(x), v(x)\big)$ rotated by 45 degrees (see \cite[Chapter 5.1]{Romik2015a}). Moreover the function $G(x)$ is continuous and increasing in the first coordinate and decreasing in the second coordinate. 
...
\begin{theorem}
\label{thm:romthm}
For each $x$ the position $\square_n(x)$, after scaling by $\sqrt{n}$, converges in probability to a specific point $G(x)\in [0,2]^2$, when $n$ tends to infinity: $$\frac{\square_n(x)}{\sqrt{n}}\overset{p}{\to}G(x).$$ 
\end{theorem}
...
The function $G(x)$ is \emph{increasing in the relation $\prec$}, i.e. if $x_1\leq x_2$ then $G(x_1)\prec G(x_2)$. Likewise from a property of the RSK insertion step for each natural number $n\in \mathbb{N}$, the function $\square_n(x)$ is increasing in the relation $\prec$, i.e. if $x_1\leq x_2$ then $\square_n(x_1)\prec \square_n(x_2)$.

\subsection{The random increasing sequence}
\label{incseq}
Let $w\in(0,1]$ be a fixed number and let $\{X'_j\}_{j=1}^{m'}$ be a finite sequence of independent random variables with the uniform distribution $U(0,w)$. We define the function $z:\{1, 2, \ldots m'\}\to [0,w]$ that assigns to a number $t\in\{1, 2, \ldots, m'\}$, the $t$-th smallest number among $X'_1, X'_2, \ldots, X'_{m'}$. The sequence $\{z(j)\}_{j=1}^{m'}$ is called the order statistic of the sequece $\{X'_j\}_{j=1}^{m'}$. 


The sequence $\{z(j)\}_{j=1}^{m'}$ contains all elements of the sequence $\{X'_j\}_{j=1}^{m'}$ in the ascending order. The sequence $\{z(j)\}_{j=1}^{m'}$ will be called a \emph{random increasing sequence with the uniform distribution on the interval $[0,w]$}. In addition, the sequence $\{X'_j\}_{j=1}^{m'}$ is a some permutation $\Pi$ of the sequence $\{z(j)\}_{j=1}^{m'}$. The permutation $\Pi$ is the random permutation with the uniform distribution. Let $\Pi=\left(\Pi_1, \Pi_2, \ldots, \Pi_{m'}\right)$. Then
$$\Big\{X'_j\Big\}_{j=1}^{m'}=\Big\{z(\Pi_j)\Big\}_{j=1}^{m'}=z\circ\Pi,$$
where $z$ is the function that acts pointwise on every element of the permutation $\Pi$. We see that the finite sequence of independent random variables with the uniform distribution $U(0,w)$ have the same distribution as the finite random increasing sequence with the uniform distribution on the interval $[0,w]$ permuted by the random permutation with the uniform distribution.
...
\begin{theorem}
\label{thm:mainthm}
For each number $A\in[1,\infty)$ the random variable $\Pos_n(\floor{An})$, after scaling by $\sqrt{wn}$, converges in probability to the limit $H(A)$, when $n$ tends to infinity. In other words, for each $\epsilon>0$: $$	\lim_{n\to\infty}\mathbb{P}\Bigg( \left\|\frac{\Pos_n\left(\floor{An}\right)}{\sqrt{wn}}-H\left(A\right)\right\|>\epsilon\Bigg)=0.$$
\end{theorem}
\begin{proof}
...
Let $n'=n'(n)$ and $m'=m'(n)$ denote the number of elements, respectively, of the sequences $\{X_j\}_{j=1}^n$, $\{X_j\}_{j=1}^m$ which are smaller than $w$. Then there is an equality:
\begin{align*}
...
From a property of the random increasing sequence with the uniform distribution (\cref{incseq}) we have:
$$\Big\{X'_j\Big\}_{j=1}^{m'}=\Big\{z(\Pi_j)\Big\}_{j=1}^{m'}=z\circ\Pi,$$
where $\{z(j)\}_{j=1}^{m'}$ is the random increasing sequence and $\Pi$ is a random permutation of range $m'$ with the uniform distribution. The function $z$ acts pointwise on every element of the permutation $\Pi$. 
...
\end{align*}
The sequence $\{Y_j\}_{j=1}^{m'}$ is a sequence of independent random variables with the uniform distribution $U(0,1)$, and the random variable $T_n$ converges almost surely to $\frac{1}{A}$:
\begin{align*}
...
\\&=\square_{m'}(T_n),
\end{align*}
where almost surely $T_n \to T=\frac{1}{A}\in(0,1)$, when $n$ tends to infinity.\\

Let $$G_{n}(x):=\frac{\square_{m'}(x)}{\sqrt{Anw}}.$$ \\
The function $\square_{m'}$ is increasing in the order $\prec$ (\cref{order}).
Hence $G_{n}$ also is increasing in the order $\prec$, i.e. if $x_1\leq x_2$, then
\begin{align}
G_{n}(x_1)\prec G_{n}(x_2).
\label{ineq}
\end{align}
From \cref{thm:romthm} for each $x\in(0,1)$ the random variable $G_n(x)$ converges in probability to the limit $G(x)$, when $n$ tends to infinity. Therefore
\begin{align*}
G_{n}(x)=\sqrt{\frac{m'}{n}\frac{1}{Aw}} \hspace{5pt} \frac{\square_{m'}(x)}{\sqrt{m'}}\overset{p}{\to} \sqrt{\frac{Aw}{Aw}}\hspace{5pt}G(x)=G(x).
\end{align*}
Using little-$o$ notation this means that for each $x\in[0,1]$ and for each $\epsilon>0$ occur: 
...
\end{align}
If $\left\|G_n(x)-G(x)\right\|\leq \epsilon$ then also
\begin{align}
(-\epsilon,\epsilon)\prec G_n(x)-G(x)\prec (\epsilon,-\epsilon).
\label{convx}
\end{align}
Let $\epsilon>0$.\\
The function $G(x)$ is a continuous function in the point $T\in(0,1)$. This means that there exists $\delta>0$ that if $|x-T| < 2\delta$ then $\left\|G(x)-G(T)\right\|<\epsilon$. Thus 
\begin{align}
(-\epsilon,\epsilon)\prec G(x)-G(T)\prec (\epsilon,-\epsilon).
\label{conti}
\end{align} 
In addition, almost surely $T_n \to T$. Then also $T_n \overset{p}{\to} T$. Using little-$o$ notation we have:
\begin{align}
\mathbb{P}(T-\delta<T_n< T+\delta)=1-o(1). 
\label{convt}
\end{align}
Furthermore if $T_n<T+\delta$ and $\left\| G_n(T+\delta)-G(T+\delta) \right\| \leq \epsilon$ then:
\begin{align*}
G_n(T_n)-G(T)&\prec G_n(T+\delta)-G(T) \tag{\ref{ineq}}\\&=G_n(T+\delta)-G(T+\delta)+G(T+\delta)-G(T)\\&\prec (\epsilon,-\epsilon)+(\epsilon,-\epsilon) \tag{\ref{convx},\ref{conti}}\\&=(2\epsilon,-2\epsilon).
\end{align*}
Analogously if $T-\delta<T_n$ and $\left\| G_n(T-\delta)-G(T-\delta) \right\|\leq \epsilon$ then:
\begin{align*}
G_n(T_n)-G(T)&\succ G_n(T-\delta)-G(T)\tag{\ref{ineq}} \\&=G_n(T-\delta)-G(T-\delta)+G(T-\delta)-G(T)\\&\succ (-\epsilon,\epsilon)+(-\epsilon,\epsilon)\tag{\ref{convx},\ref{conti}}\\&=(-2\epsilon,2\epsilon).
\end{align*}
Thus if $\left\| G_n(T+\delta)-G(T+\delta) \right\| \leq \epsilon$ and $\left\| G_n(T-\delta)-G(T-\delta) \right\|\leq \epsilon$ and \linebreak[4]{$\left| T_n-t\right|<\delta$} then:
\begin{align*}
\left\|G_n(T_n)-G(T)\right\|< \left\| (2\epsilon,-2\epsilon) \right\| <4 \epsilon .
\end{align*}
Thereby:
\begin{align*}
&\mathbb{P}\bigg(\left\|G_{n}(T_n)-G(T)\right\| > 4\epsilon\bigg)\leq \\&\leq  \mathbb{P}\Big(\left\|G_n(T+\delta)-G(T+\delta)\right\|> \epsilon \text{\hspace{15pt}or\hspace{12pt}}\left\|G_n(T-\delta)-G(T-\delta)\right\|> \epsilon \text{\hspace{15pt}or\hspace{12pt}}\left| T_n-T\right|\geq \delta\Big)\\&\leq  \mathbb{P}\Big(\left\|G_n(T+\delta)-G(T+\delta)\right\|> \epsilon \Big) +\mathbb{P}\Big( \left\|G_n(T-\delta)-G(T-\delta)\right\|> \epsilon\Big)+ \mathbb{P}\Big(\left| T_n-T\right|\geq \delta\Big)\\&=o(1).\tag{\ref{smallo},\ref{convt}}
\end{align*}
Then for each $A\in[1,\infty)$ and each $\epsilon>0$ we have:
$$\lim_{n\to\infty}\mathbb{P}\bigg(\left\|G_{n}(T_n)-G(T)\right\| > 4\frac{\epsilon}{4\sqrt{A}}\bigg)=0,$$
$$\lim_{n\to\infty}\mathbb{P}\left(\left\|\frac{\Pos_n\left(\floor{An}\right)}{\sqrt{wn}}-H\left(A\right)\right\|>\epsilon\right)=0.$$


\end{proof}

\section{The uniform convergence on the interval}

\begin{theorem}
For each  number $A\in[1,R]$ the random variable $\Pos_n(\floor{An})$, after scaling by $\sqrt{wn}$, 
converges in probability to the limit $H(A)$, when $n$ tends to infinity. Moreover, the convergence is uniform, i.e.  for each $\epsilon>0$

$$	\lim_{n\to\infty}\mathbb{P}\Bigg(\sup_{A\in[1,R]} \left\|\frac{\Pos_n\left(\floor{An}\right)}{\sqrt{wn}}-H\left(A\right)\right\|>\epsilon\Bigg)=0.$$

\end{theorem}

\begin{proof}
From proof of \cref{thm:mainthm} we know that $G_n(T_n)$ converges in probability to the limit $G(T)$, when $n$ tends to infinity for $T=\frac{1}{A}\in[\frac{1}{R},1]$.\\
The function $G(x)$ is also increasing in the order $\prec$. In addition $G(x)$ is the continuous function on the closed interval $[\frac{1}{R},1]$. Then from the Heine-Cantor theorem the function $G(x)$ is uniformly continuous on the interval $[\frac{1}{R},1]$. 
This means that for each $\epsilon>0$ there exists $\delta>0$ that for each $x\in[\frac{1}{R}, 1]$ if $|x-T| < \delta$ then $\left\|G(x)-G(T)\right\|<\epsilon$. Thus 

\begin{align*}
(-\epsilon,\epsilon)\prec G(x)-G(T)\prec (\epsilon,-\epsilon).
\end{align*}

Let $\delta>0$. We split the interval $[\frac{1}{R}, 1]$ into the $p$ equal intervals of lengths $\frac{1-\frac{1}{R}}{p}<\delta$ for a some natural number $p$. Let $x_0, x_1, \ldots, x_n$ denote the ends of these intervals. Let $T_n\in[\frac{1}{R},1]$. We choose a number $n_0$ so that 
\begin{align*}
T_n, T\in(x_k, x_{k+1})\text{ for a some number }k\in\{0, 1, \ldots, p-1\} 
\end{align*}
and
\begin{align*}
||G_n(x_k) -G(x_k)||<\epsilon \text{ for all }n>n_0\text{ and all } k\in\{0, 1, \ldots, p\}. 
\end{align*}
Thereby
\begin{align*}
G_n(T_n)-G(T)&\prec G_n(x_{k+1})-G(T) \tag{\ref{ineq}}\\&\prec(\epsilon,-\epsilon)+G(x_{k+1})-G(T)\\&\prec (\epsilon,-\epsilon)+(\epsilon,-\epsilon) \tag{\ref{ineq}}\\&=(2\epsilon,-2\epsilon).
\end{align*}
Analogously
\begin{align*}
G_n(T_n)-G(T)&\succ G_n(x_{k})-G(T) \tag{\ref{ineq}}\\&\succ(-\epsilon,\epsilon)+G(x_{k})-G(T)\\&\succ (-\epsilon,\epsilon)+(-\epsilon,\epsilon) \tag{\ref{ineq}}\\&=(-2\epsilon,2\epsilon).
\end{align*}
Thus
\begin{align*}
\left\|G_n(T_n)-G(T)\right\|< \left\| (2\epsilon,-2\epsilon) \right\| <4 \epsilon .
\end{align*}
Obviously if $T_n=x_k$ for a some number $k\in{0, 1, \ldots, p}$ then the inequality also is fulfil. Then for each $A\in[1,R]$ and each $\epsilon>0$ we have:
$$\lim_{n\to\infty}\mathbb{P}\bigg(\sup_{T\in[\frac{1}{R},1]}\sqrt{A}\left\|G_{n}(T_n)-G(T)\right\| > 4\sqrt{A}\epsilon\bigg)=0.$$
Using the inequality $A\leq R$ we obtain

$$\lim_{n\to\infty}\mathbb{P}\bigg(\sup_{T\in[\frac{1}{R},1]}\sqrt{A}\left\|G_{n}(T_n)-G(T)\right\| > 4\sqrt{R}\epsilon\bigg)=0,$$
$$	\lim_{n\to\infty}\mathbb{P}\Bigg(\sup_{A\in[1,R]} \left\|\frac{\Pos_n\left(\floor{An}\right)}{\sqrt{wn}}-H\left(A\right)\right\|>4\sqrt{R}\epsilon\Bigg)=0.$$
Thus for each $\epsilon>0$ occur
$$	\lim_{n\to\infty}\mathbb{P}\Bigg(\sup_{A\in[1,R]} \left\|\frac{\Pos_n\left(\floor{An}\right)}{\sqrt{wn}}-H\left(A\right)\right\|>\epsilon\Bigg)=0.$$
\end{proof}

\section{Asymptotic behavior of the function $H(A)$}

Let $A=\frac{1}{x}$. In this chapter we will describe the asymptotic behavior of the function $H\big(\frac{1}{x}\big)$ when $x$ tends to zero. For this purpose, we will use the formulas given in \cref{wzory}.

$$H\big(1/x\big)=\sqrt{\frac{1}{x}}G(x)=\frac{1}{2\sqrt{x}}\bigg(v(x)+u(x), v(x)-u(x)\bigg).$$
We define the function $f(x):[0,2]\to [0,\sqrt{\frac{3\pi}{2}}]$ by
$$f(x)=\sqrt[3]{\frac{3\pi}{2} F_{sc}(x^2-2)}.$$
The function $F_{sc}$ is inversible, so the function $f$ also is inversible and the above formula can be transformed into a form
$$f^{-1}(y)=\sqrt{F_{sc}^{-1}\bigg(\frac{2}{3\pi}y^3\bigg)+2}.$$
Therefore for $x\in[0,1]$ we have:
$$F_{sc}^{-1}(x)=\Bigg(f^{-1}\bigg(\sqrt[3]{\frac{3\pi}{2}x}\bigg)\Bigg)^{2}-2.$$
Next we determine the taylor series of the function $f(x)$.
$$f(x)=x-\frac{1}{40}x^3-\frac{39}{22400}x^5-O(x^7).$$
We use some formulas and determine the series of inverse function $f^{-1}(y)$.
$$f^{-1}(y)=y+\frac{1}{40}y^3+\frac{81}{22400}y^5+O(y^7).$$
Of course $O\big(f^{-1}(y)\big)=O(y)$.
Then for $x\in[0,1]$ occurs
\begin{align*}
F_{sc}^{-1}(x)&=\left(\left(\sqrt[3]{\frac{3\pi}{2}x}\right)+\frac{1}{40}\left(\sqrt[3]{\frac{3\pi}{2}x}\right)^3+\frac{81}{22400}\left(\sqrt[3]{\frac{3\pi}{2}x}\right)^5+O\left(\sqrt[3]{\frac{3\pi}{2}x}\right)^7\right)^2-2\\&=-2+\left(\frac{3\pi}{2}x\right)^{\frac{2}{3}}+\frac{1}{20}\left(\frac{3\pi}{2}x\right)^{\frac{4}{3}}+\frac{11}{1400}\left(\frac{3\pi}{2}x\right)^{\frac{6}{3}}+O\left(x^{\frac{8}{3}}\right)
\end{align*}
Additionally we determine the taylor series of the function $\Omega_{\star}(t^2-2)+t^2-2$
$$\Omega_{\star}(t^2-2)+t^2-2=\frac{4}{3\pi}t^3+\frac{1}{30\pi}t^5+\frac{3}{1120\pi}t^7+O\left(t^{9}\right)$$
Thereby
\begin{align*}
&v(x)+u(x)\\&=\Omega_{\star}\big(u(x)\big)+u(x)\\&=\Omega_{\star}\big(F_{sc}^{-1}(x)\big)+F_{sc}^{-1}(x)\\&=\Omega_{\star}\bigs(\Bigg(f^{-1}\bigg(\sqrt[3]{\frac{3\pi}{2}x}\bigg)\Bigg)^{2}-2\bigs)+\Bigg(f^{-1}\bigg(\sqrt[3]{\frac{3\pi}{2}x}\bigg)\Bigg)^{2}-2\\
&=\frac{4}{3\pi}\Bigg(f^{-1}\bigg(\sqrt[3]{\frac{3\pi}{2}x}\bigg)\Bigg)^3+\frac{1}{30\pi}\Bigg(f^{-1}\bigg(\sqrt[3]{\frac{3\pi}{2}x}\bigg)\Bigg)^5+\frac{3}{1120\pi}\Bigg(f^{-1}\bigg(\sqrt[3]{\frac{3\pi}{2}x}\bigg)\Bigg)^7+O\left(x^{\frac{9}{3}}\right)\\
&=O\left(x^{\frac{9}{3}}\right)+\frac{4}{3\pi}\Bigg(\bigg(\frac{3\pi}{2}x\bigg)^{\frac{1}{3}}+\frac{1}{40}\bigg(\frac{3\pi}{2}x\bigg)^{\frac{3}{3}}+\frac{81}{22400}\bigg(\frac{3\pi}{2}x\bigg)^{\frac{5}{3}}+O\left(x^{\frac{7}{3}}\right)\Bigg)^3+...\\
&=O\left(x^{\frac{9}{3}}\right)+\frac{4}{3\pi}\Bigg(\bigg(\frac{3\pi}{2}x\bigg)^{\frac{1}{3}}+\frac{1}{40}\bigg(\frac{3\pi}{2}x\bigg)^{\frac{3}{3}}+\frac{81}{22400}\bigg(\frac{3\pi}{2}x\bigg)^{\frac{5}{3}}\Bigg)^3+...\\
&=2x+\frac{1}{5}\left(\frac{3\pi}{2}\right)^{\frac{2}{3}}x^{\frac{5}{3}}+\frac{1}{28}\left(\frac{3\pi}{2}\right)^{\frac{4}{3}}x^{\frac{7}{3}}+O\left(x^3\right)
\end{align*}
and
\begin{align*}
&v(x)-u(x)\\&=v(x)+u(x)-2u(x)\\&=v(x)+u(x)-2F_{sc}^{-1}(x)\\&=2x+\frac{1}{5}\left(\frac{3\pi}{2}\right)^{\frac{2}{3}}x^{\frac{5}{3}}+\frac{1}{28}\left(\frac{3\pi}{2}\right)^{\frac{4}{3}}x^{\frac{7}{3}}+4-2\left(\frac{3\pi}{2}x\right)^{\frac{2}{3}}-\frac{1}{10}\left(\frac{3\pi}{2}x\right)^{\frac{4}{3}}-\frac{11}{700}\left(\frac{3\pi}{2}x\right)^{\frac{6}{3}}+O\left(x^{\frac{8}{3}}\right)
\end{align*}
Thus
\begin{align*}
\frac{v(x)+u(x)}{2\sqrt{x}}&=\sqrt{x}+\frac{1}{10}\left(\frac{3\pi}{2}\right)^{\frac{2}{3}}x^{\frac{7}{6}}+\frac{1}{56}\left(\frac{3\pi}{2}\right)^{\frac{4}{3}}x^{\frac{11}{6}}+O\left(x^{\frac{15}{6}}\right)
\end{align*}
and

%%\begin{align*}
%%&\frac{v(x)-u(x)}{2\sqrt{x}}\\&=\frac{2}{\sqrt{x}}-\left(\frac{3\pi}{2}\right)^{\frac{2}{3}}\hspace{-5pt}x^{\frac{1}{6}}+x^{\frac{3}{6}}-\frac{1}{20}\left(\frac{3\pi}{2}\right)^{\frac{4}{3}}\hspace{-5pt}x^{\frac{5}{6}}+\frac{1}{10}\left(\frac{3\pi}{2}\right)^{\frac{2}{3}}\hspace{-5pt}x^{\frac{7}{6}}-\frac{11}{1400}\left(\frac{3\pi}{2}\right)^{\frac{6}{3}}\hspace{-5pt}x^{\frac{9}{6}}+\frac{1}{56}\left(\frac{3\pi}{2}\right)^{\frac{4}{3}}\hspace{-5pt}x^{\frac{11}{6}}+O\left(x^{\frac{13}{6}}\right)
%%\end{align*}

\begin{align*}
&\frac{v(x)-u(x)}{2\sqrt{x}}\\&=
\frac{2}{\sqrt{x}}-\left(\frac{3\pi}{2}\right)^{\frac{2}{3}}\hspace{-5pt}x^{\frac{1}{6}}+x^{\frac{3}{6}}-\left(\frac{3\pi}{2}\right)^{\frac{4}{3}}\frac{x^{\frac{5}{6}}}{20}+\left(\frac{3\pi}{2}\right)^{\frac{2}{3}}\frac{x^{\frac{7}{6}}}{10}-\left(\frac{3\pi}{2}\right)^{\frac{6}{3}}\frac{11x^{\frac{9}{6}}}{1400}+\left(\frac{3\pi}{2}\right)^{\frac{4}{3}}\frac{x^{\frac{11}{6}}}{56}+O\left(x^{\frac{13}{6}}\right)
\end{align*}
