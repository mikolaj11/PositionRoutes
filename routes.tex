...
\label{ineq}
\end{align}
Therefore from \cref{thm:romthm} for each $x\in(0,1)$ the random variable $G_n(x)$ converges in probability to the limit $G(x)$, when $n$ tends to infinity:
\begin{align*}
...
\end{align}
Let $\epsilon>0$.\\
The function $G(x)$ is a continuous function in the point $T\in(0,1)$. This means that exists $\delta>0$ that if $|x-T| < 2\delta$ then $\left\|G(x)-G(T)\right\|<\epsilon$. Thus 
\begin{align}
-(\epsilon,-\epsilon)\prec G(x)-G(T)\prec (\epsilon,-\epsilon).
\label{conti}
\end{align} 
In addition, almost surely $T_n \to T$. Then also $T_n \overset{p}{\to} T$. Using little-$o$ notation e have:
\begin{align}
\mathbb{P}(T-\delta<T_n< T+\delta)=1-o(1). 
\label{convt}
\end{align}
Furthermore if $T_n<T+\delta$ and $\left\| G_n(T+\gamma)-G(T+\gamma) \right\| \leq \epsilon$ then:
\begin{align*}
G_n(T_n)-G(T)&\prec G_n(T+\gamma)-G(T) \tag{\ref{ineq}}\\&=G_n(T+\gamma)-G(T+\gamma)+G(T+\gamma)-G(T)\\&\prec (\epsilon,-\epsilon)+(\epsilon,-\epsilon) \tag{\ref{convx},\ref{conti}}\\&=(2\epsilon,-2\epsilon).
\end{align*}
Analogously if $T-\delta<T_n$ and $\left\| G_n(T-\gamma)-G(T-\gamma) \right\|\leq \epsilon$ then:
\begin{align*}
G_n(T_n)-G(T)&\succ G_n(T-\gamma)-G(T)\tag{\ref{ineq}} \\&=G_n(T-\gamma)-G(T-\gamma)+G(T-\gamma)-G(T)\\&\succ -(\epsilon,-\epsilon)-(\epsilon,-\epsilon)\tag{\ref{convx},\ref{conti}}\\&=-(2\epsilon,-2\epsilon).
\end{align*}
Thus if $\left\| G_n(T+\gamma)-G(T+\gamma) \right\| \leq \epsilon$ and $\left\| G_n(T-\gamma)-G(T-\gamma) \right\|\leq \epsilon$ and \linebreak[4]{$\left\| T_n-t\right\|<\gamma$} then:
\begin{align*}
\left\|G_n(T_n)-G(T)\right\|< \left\| (2\epsilon,-2\epsilon) \right\| <4 \epsilon .
\end{align*}
Thereby:
\begin{align*}
1&\geq\mathbb{P}\bigg(\left\|G_{n}(T_n)-G(T)\right\|\leq 4\epsilon\bigg)\\&\geq \mathbb{P}\Big(\left\|G_n(T+\gamma)-G(T+\gamma)\right\|\leq \epsilon\text{\hspace{5pt}and\hspace{5pt}} \left\|G_n(T-\gamma)-G(T-\gamma)\right\|\leq \epsilon \text{\hspace{5pt}and\hspace{5pt}} \left\| T_n-T\right\| <\gamma\Big) \\&\geq \mathbb{P}\Big(\left\|G_n(T+\gamma)-G(T+\gamma)\right\|\leq \epsilon\text{\hspace{5pt}and\hspace{5pt}} \left\|G_n(T-\gamma)-G(T-\gamma)\right\|\leq \epsilon\Big) + \mathbb{P}(\left\| T_n-T\right\|<\gamma)-1 \\&\geq \mathbb{P}\Big(\left\|G_n(T+\gamma)-G(T+\gamma)\right\|\leq \epsilon \Big) +\mathbb{P}\Big( \left\|G_n(T-\gamma)-G(T-\gamma)\right\|\leq \epsilon\Big)+ \mathbb{P}(\left\| T_n-T\right\|<\gamma)-2\\&=1-o(1)+1-o(1)+1-o(1)-2 \tag{\ref{smallo},\ref{convt}} \\&=1-o(1),
\end{align*}
