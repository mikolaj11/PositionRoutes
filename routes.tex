...
\end{center}
\begin{center}
The trajectory describing the position of the box with the fixed\\number $w$ at the moment $\floor{An}$ for numbers $A=1$, $A=2$ and $A=3$.
\end{center} 
}
...
The main result of the article is Theorem 1 describing the asymptotic behavior of the box with a fixed number. When the number of boxes tends to infinity then the trajectory of the box converges in probability to the function $H(A)=\sqrt{A}G(\frac{1}{A})$. 
\trzy
...
\begin{theorem}
For each number $A\in(1,\infty)$ the random variable $\Pos_n(\floor{An})$, after scaling by $\sqrt{wn}$ converges in probability to the limit $H(A)$, when $n$ tends to infinity: $$\forall_{\epsilon>0} \lim_{n\to\infty}\mathbb{P}\left( \left\|\frac{\Pos_n\left(\floor{An}\right)}{\sqrt{wn}}-H\left(A\right)\right\|<\epsilon\right)=1$$
\end{theorem}
\begin{proof}
...
Now we're gonna use this observation in the proof. Let $m=\floor{An}$. The probability that the same number will occur twice in the sequence $(w, X_1, X_2, \ldots)$ is equal to 0, hence without losing generality we assume that the numbers $w, X_1, X_2, \ldots$ are all different. Let the sequence $\{X'_j\}_{j=1}^\infty$ be the subsequence of the sequence $\{X_j\}_{j=1}^\infty$ containing all elements of the sequence $\{X_j\}_{j=1}^\infty$, which are less than $w$. The sequence $\{X'_j\}_{j=1}^{\infty}$ is a sequence of independent random variables with uniform distribution $U(0,w)$ random variables. Let $n'=n'(n)$ and $m'=m'(m)$ denote the numbers of elements of the sequences $\{X_j\}_{j=1}^n$, $\{X_j\}_{j=1}^m$ which are smaller than $w$. Then there is an equality:
\begin{align*}
\Pos_n\left(\floor{An}\right)&=\boxi_w\left(P\left(X_1, \ldots, X_n, w, X_{n+1}, \ldots, X_{m_n}\right)\right)\\&=\boxi_w\left(P\left(X_1, \ldots, X'_{n'}, w, X'_{n'+1}, \ldots, X'_{m'}\right)\right).
\end{align*}
...
From Romik and Śniady for each $x\in(0,1)$  we have:
\begin{align*}
...
\end{align*}
We know that if $n$ tends to infinity then with probability $1$ for each $x$ occurs:
$$
\left\{
\begin{matrix}
...
\right.
$$
In addition the function $G(x)$ is monotonic and Lipschitz function with constant $1$. Then:
