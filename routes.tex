...
The function $G(x) \colon [0,1]\to[0,2]^2$ is a certain special parameterization of the Logan--Sheep--Vershik--Kerov curve rotated by 45 degrees. \cref{fig:gfunction} shows the graph of the function $G(x)$. Moreover the function $G(x)$ is continuous and increasing due to the first coordinate and decreasing due to the second coordinate. \\
...
We define \emph{the partial order on the plane $\prec$} as follows:
$(x_1,y_1)\prec(x_2,y_2)$ if and only if $x_1\leq x_2$ and $y_1\geq y_2$.\\
From a property of the Young tableau and the RSK inserion step we know that if $x_1\leq x_2$ then $$\square_{m'}(x_1)\prec \square_{m'}(x_2),$$
Hence
\begin{align}
