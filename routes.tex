...
In addition, the sequence $\{X'_j\}_{j=1}^{m'}$ is a some permutation $\Pi=\left(\Pi_1, \Pi_2, \ldots, \Pi_{m'}\right)$ of the sequence $\{z(j)\}_{j=1}^{m'}$. Thus
...
\end{align}
\\
Let $\epsilon>0$. The function $G(x)$ is a continuous function in the point $A=\frac{1}{T}$. This means that there exists $\delta>0$ that if $|x-A| < 2\delta$ then $\left\|G(x)-G(A)\right\|<\epsilon$. Thus 
\begin{align}
...
Here we finished the proof of the pointwise convergence.
\end{proof}

\subsection{The uniform convergence on the interval}

In this section we prove the uniform convergence on the interval $[1, \infty)$. 
\begin{proof}
...
\left\|G_n(A_n)-G(A)\right\|< \left\| (2\epsilon,-2\epsilon) \right\| <4 \epsilon .
\end{align*}
Obviously if $A=x_k$ for a some number $k\in\{0, 1, \ldots, p\}$ then the inequality also is fulfilled. Then for each $A\in[1,R]$ and each $\epsilon>0$ we have:
...
In this chapter we will describe the asymptotic behavior of the function $H\left(T\right)=\left(H_1\left(T\right), H_2\left(T\right)\right)$ when $T$ tends to infinity. Let $A=\frac{1}{T}$. For this purpose, we will use the formulas given in \cref{wzory}.
...
$$F_{sc}^{-1}(x)=\left(f^{-1}\left(\sqrt[3]{\frac{3\pi}{2}x}\right)\right)^{2}-2.$$
Next we calculate directly the derivatives of the function $f$ and thus we determine the Taylor series of the function $f$ at the point zero
$$f(x)=x-\frac{1}{40}x^3-\frac{39}{22400}x^5-O(x^7).$$
We use the Lagrange inversion formula for the coefficients of the series of the inverse function given by Morse and Feshbach in \cite{Morse1953} and we determine the series of inverse function $f^{-1}(y)$.
...
Additionally we calculate directly the derivatives and we determine the Taylor series of the function $\Omega_{\star}(t^2-2)+t^2-2$ at the point zero
...
Thus
\begin{align*}
H_1(T)&=\frac{v(A)+u(A)}{2\sqrt{A}}\\&=\frac{1}{\sqrt{T}}+\frac{1}{10}\left(\frac{3\pi}{2}\right)^{\frac{2}{3}}\left(\frac{1}{T}\right)^{\frac{7}{6}}+\frac{1}{56}\left(\frac{3\pi}{2}\right)^{\frac{4}{3}}\left(\frac{1}{T}\right)^{\frac{11}{6}}+O\left(\left(\frac{1}{T}\right)^{\frac{15}{6}}\right)
\end{align*}
and
