...
In the proof of theorem \ref{thm:mainthm} we will need the following theorem 5.1 in \cite{Romik2015} proved by Romik and Śniady. 
\begin{theorem}
\label{thm:romthm}
...
\label{fig:gfunction}
\end{figure}
The figure~\ref{fig:gfunction} shows the graph of the function $G(x)$. Moreover the Logan--Sheep--Vershik--Kerov curve \cite{LoganShepp1977}\cite{VershikKerov1986} is a Lipschitz function with constant $1$. After it is rotated means that for each $x\geq 0$ and $\epsilon>0$ where $x+\epsilon\leq 1$ we have:
$$G(x)-\epsilon(1,-1)\prec G(x+\epsilon) \prec G(x)+\epsilon(1,-1),$$
where the relation $<$ is an order on the plane defined as follows:
$(x_1,y_1)\prec(x_2,y_2)$ only and only if $x_1<x_2$ and $y_1>y_2$. The relation $\prec$ retains the order of the points on the plane along the line $y=-x$. In addition, the function $G(x)$ is also monotonic with respect to the relation $<$ on the plane.
\end{theorem}
...
\end{align*}
The random variable $n'=\sum_{j=1}^{n}[X_j<w]$ counts how many numbers from the sequence $\{X_j\}_{j=1}^{n}$ are less than $w$, so $n'$ is a random variable with the binomial distribution with parameters $n$ and $w$. We denote it $n'\sim B(n,w)$. Likewise the random variable $m'-n'$ counts how many numbers from the sequence $\{X_j\}_{j=n'+1}^{m'}$ are less than $w$, so $m'-n'\sim B(m-n,w)$. Moreover, the random variables $n'$ and $m'-n'$ are independent, because the random variables $X_1, X_2, \ldots$ are independent. \\
From the Strong Law of Large Numbers theorem \cite{Durrett2019}, we know that if $n$ tends to infinity, then the limits exist with probability $1$:
$$
\begin{matrix}
