...
\begin{theorem}
\label{thm:romthm}
For each $x\in[0,1]$ the position $\square_n(x)$, after scaling by $\sqrt{n}$, converges in probability to a specific point $G(x)\in [0,2]^2$, when $n$ tends to infinity: $$\frac{\square_n(x)}{\sqrt{n}}\overset{p}{\to}G(x).$$ 
\end{theorem}
...
\end{align*}
Now, by changing the number $n'+1$ to the number $n'$ we get the inverse permutation $\Pi^{-1}$ 
\begin{align*}
...
\end{align*}
and using the function $z$, we will get a sequence of independent random variables with the uniform distribution $U(0,1)$ 
\pagebreak[2]
\begin{align*}
...
\end{align*}
We see that the function $H(T)$ behaves asymptotically like the function $(\frac{1}{\sqrt{T}}, 2\sqrt{T})$
