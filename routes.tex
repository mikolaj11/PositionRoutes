...
The function $G(x) \colon [0,1]\to[0,2]^2$ is a certain special parameterization of the Logan--Sheep--Vershik--Kerov curve rotated by 45 degrees (see \cite{Romik2015a}). \cref{fig:gfunction} shows the graph of the function $G(x)$. Moreover the function $G(x)$ is continuous and increasing due to the first coordinate and decreasing due to the second coordinate. \\
...
\begin{theorem}
\label{thm:romthm}
The position $\square_n(x)$, after scaling by $\sqrt{n}$, converges in probability to a specific point $G(x)\in [0,2]^2$: $$\frac{\square_n(x)}{\sqrt{n}}\overset{p}{\to}G(x).$$ 
\end{theorem}

\subsection{The partial order on the plane}
\label{order}
We define \emph{the partial order on the plane $\prec$} as follows:
$(x_1,y_1)\prec(x_2,y_2)$ if and only if $x_1\leq x_2$ and $y_1\geq y_2$. \\

The function $G(x)$ is \emph{monotonic due to the relation $\prec$}. It means that if $x_1\leq x_2$ then $G(x_1)\prec G(x_2)$. Likewise from a property of the RSK insertion step for each natural number $n\in \mathbb{N}$, the function $\square_n(x)$ is monotonic due to the relation $\prec$. It means that if $x_1\leq x_2$ then $\square_n(x_1)\prec \square_n(x_2)$.

\subsection{The random increasing sequence}
\label{incseq}
Let $w\in(0,1]$ be a fixed number and let $\{X'_j\}_{j=1}^{m'}$ be a sequence of independent random variables with the uniform distribution $U(0,w)$. We define the function $z:\{1, 2, \ldots m'\}\to [0,w]$ that assigns to a number $t\in\{1, 2, \ldots, m'\}$, the $t$-th smallest number among $X'_1, X'_2, \ldots, X'_{m'}$.


The sequence $\{z(j)\}_{j=1}^{m'}$ contains all elements of the sequence $\{X'_j\}_{j=1}^{m'}$ in the ascending order. In addition, the sequence $\{X'_j\}_{j=1}^{m'}$ is a random permutation $\Pi$ with the uniform distribution of the sequence $\{z(j)\}_{j=1}^{m'}$. The sequence $\{z(j)\}_{j=1}^{m'}$ will be called a \emph{random increasing sequence with the uniform distribution on the interval $[0,w]$}. Let $\Pi=\left(\Pi_1, \Pi_2, \ldots, \Pi_{m'}\right)$. Then
$$\Big\{X'_j\Big\}_{j=1}^{m'}=\Big\{z(\Pi_j)\Big\}_{j=1}^{m'}=z\circ\Pi,$$
where $z$ is the function that acts pointwise on every element of the permutation $\Pi$. We see that the finite sequence of independent random variables with the uniform distribution $U(0,w)$ is the finite random increasing sequence with the uniform distribution on the interval $
[0,w]$ permuted by the random permutation.
...
From \cref{incseq} we have:
$$\Big\{X'_j\Big\}_{j=1}^{m'}=\Big\{z(\Pi_j)\Big\}_{j=1}^{m'}=z\circ\Pi,$$
where $\{z(j)\}_{j=1}^{m'}$ is the random increasing sequence and $\Pi$ is a random permutation. The function $z$ acts pointwise on every element of the permutation $\Pi$. 
...
From the \cref{order} we know that if $x_1\leq x_2$ then: $$\square_{m'}(x_1)\prec \square_{m'}(x_2),$$
Hence
\begin{align}
G_{n}(x_1)\prec G_{n}(x_2).
\label{ineq}
\end{align}
From \cref{thm:romthm} for each $x\in(0,1)$ the random variable $G_n(x)$ converges in probability to the limit $G(x)$, when $n$ tends to infinity:
\begin{align*}
...
Thereby:
\begin{align*}
&\mathbb{P}\bigg(\left\|G_{n}(T_n)-G(T)\right\| > 4\epsilon\bigg)\leq \\&\leq  \mathbb{P}\Big(\left\|G_n(T+\delta)-G(T+\delta)\right\|> \epsilon \text{\hspace{15pt}or\hspace{12pt}}\left\|G_n(T-\delta)-G(T-\delta)\right\|> \epsilon \text{\hspace{15pt}or\hspace{12pt}}\left\| T_n-T\right\|\geq \delta\Big)\\&\leq  \mathbb{P}\Big(\left\|G_n(T+\delta)-G(T+\delta)\right\|> \epsilon \Big) +\mathbb{P}\Big( \left\|G_n(T-\delta)-G(T-\delta)\right\|> \epsilon\Big)+ \mathbb{P}\Big(\left\| T_n-T\right\|\geq \delta\Big)\\&=o(1).
\end{align*}
